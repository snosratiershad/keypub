...
INSERT INTO
    verification_codes (email, fingerprint, code)
VALUES
    (?, ?, ?) RETURNING *;
...
INSERT INTO
    admin_fingerprints (fingerprint)
VALUES
    (?) RETURNING *;
...
INSERT INTO
    email_permissions (granter_email, grantee_email)
VALUES
    (?, ?) RETURNING *;
...
-- name: GetTargetInfoWithFingerprintAndGranterEmailAndGranteeEmail :many
SELECT
    ssh_keys.email
FROM
    ssh_keys
    LEFT JOIN email_permissions ON email_permissions.granter_email = ssh_keys.email
    AND email_permissions.grantee_email = ?
WHERE
